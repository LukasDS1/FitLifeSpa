...
public class LoadDataBase {
    ...
    @Bean
    CommandLineRunner initDataBase(EstadoRepository estadoRepo, PrivilegesRepository privRepo, ModuloRepository modRepo,RolRepository rolRepository) {
        return args -> {
            if (estadoRepo.count() == 0 && privRepo.count() == 0 && modRepo.count() == 0 && rolRepository.count() ==0 ) {
